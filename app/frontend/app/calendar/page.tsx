...
export default async function CalendarPage({ searchParams }: CalendarPageProps) {
  ...
  const championshipTeams = [...new Set(events.flatMap((event) => event.championship_teams ?? []))].sort((a, b) => a.localeCompare(b, "fr"));
  ...
  const visibleEvents = activeFilter === "all"
    ? events
    : events.filter((event) => eventFilter(event) === activeFilter && (!requestedTeam || (event.championship_teams ?? []).includes(requestedTeam)));
