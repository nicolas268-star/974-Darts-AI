...
type PlayerMatchStats = {
  ...
  visits100: number; visits140: number; visits170: number; visits180: number; zeroVisits: number;
...
type RoundMatch = { id: string; phase: string; scoreUnit: string; recordedDarts: number; dataStatus: string; recordedLegs: number; officialLegs: number; teamA: MatchSide; teamB: MatchSide };
type PlayerGlobalStats = PlayerMatchStats & { matches: number; duoScore: number; contribution: number | null };
type RoundDetails = {
  round: number; status: string; quality: { individualMatches: number; totalMatches: number; incompleteMatches: number; unavailableMatches: number };
...
function PlayerMatchCard({ player }: { player: PlayerMatchStats }) {
  return <article className="bdc-player-match">
    ...
    <dl className="bdc-metrics-grid">
      ...
      <div><dt>Tours sans score</dt><dd>{player.zeroVisits}</dd></div>
    </dl>
...
function MatchDetail({ match }: { match: RoundMatch }) {
  const unavailable = !match.teamA.players || !match.teamB.players;
  const incomplete = match.dataStatus === "partial-individual-incomplete";
  return <details className="bdc-match-card"><summary>
    ...
    {unavailable ? <div className="bdc-unavailable-panel"><strong>{UNAVAILABLE}</strong><p>Aucune volée individuelle ne peut être récupérée pour cette rencontre. Le score collectif et les moyennes du duo restent affichés.</p></div>
      : <>{incomplete && <div className="bdc-unavailable-panel"><strong>Données individuelles partielles</strong><p>{match.recordedLegs} legs enregistrés sur {match.officialLegs}. Les statistiques utilisent uniquement les volées conservées avant l’incident.</p></div>}<div className="bdc-duo-details">{[match.teamA, match.teamB].map(side => <section key={side.teamId}>
...
export function BdcRoundTemplate({ details, source }: { details: RoundDetails; source: RoundSource }) {
  ...
  return <section id={`resultats-manche-${details.round}`} className="bdc-section bdc-round-report">
    ...
    <section id="m1-synthese" className="bdc-report-block">
      ...
      <div className="bdc-kpis"><article><span>Champion</span><strong>Super Mario / Pierre</strong><small>11 points par joueur</small></article><article><span>Format</span><strong>8 doublettes</strong><small>16 joueurs</small></article><article><span>Rencontres</span><strong>32 matchs</strong><small>28 poule · 4 phase finale</small></article><article><span>Couverture individuelle</span><strong>{details.quality.individualMatches}/{details.quality.totalMatches}</strong><small>{details.quality.incompleteMatches} partielles · {details.quality.unavailableMatches} indisponible</small></article></div>
